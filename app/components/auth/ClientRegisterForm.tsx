...
import { UserRole, type ClientRegistration } from '~/models/auth';
...
export const ClientRegisterForm: React.FC<ClientRegisterFormProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const registrationData: ClientRegistration = {
        name,
        email,
        password,
    ...
    <Form onSubmit={handleSubmit} className="register-form">
      ...
      <div className="mt-4 text-center">
        <p className="text-sm text-gray-600">
          Já tem uma conta?{' '}
          <a href="/login/client" className="text-blue-600 hover:text-blue-800">
            Faça login
          </a>
        </p>
      </div>
